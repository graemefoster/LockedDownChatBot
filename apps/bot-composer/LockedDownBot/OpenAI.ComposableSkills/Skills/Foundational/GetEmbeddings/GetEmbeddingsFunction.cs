...
{
    public record Input(
        [property: Description("Content to get embeddings from")]
        string Content);

    public record Output(
        [property: Description("Original content")]
        string Content,
        [property: Description("A list of embeddings")]
        float[] Embeddings);
    ...
    public class Function : IChainableSkill<Input, Output>
    {
        ...
        {
            var embeddings =
                await _openAiClient.GetEmbeddingsAsync(new EmbeddingsOptions(_embeddingsModel, [input.Content]), token);
